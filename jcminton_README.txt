...
=====================================================================================================================================================================================
My Implementations - assignment and ifStatement:
------------------------------------------------
1. Assignment
	a. assignment(std::string idName) makes use of the ASTNode structure to determine the type of expression used.
	b. The function first checks for the id_use token, which already appends the left-hand side of the assignment statement.
	c. If assignment follows the parser order for the token <assignment>, the function generates a root in ASTNode and evaluates the expression.
	d. The function appends all values in the ASTNode tree to the assignment statement, followed by a semicolon.
	e. The final assignment statement is added to the list of functions.
2. If Statement
	a. ifStatement() makes use of the ASTNode structure to determine the type of condition expression used.
	b. The function checks for every token except <block statements> before continuing.
	c. The if statement generation involves a modified grammar after the expression of ) goto LABEL ;
		i. label1 is the concatenation of "c" and the current label count of the current symbol table, saved in a variable with ID label1.
		ii. The formation of each LABEL variable also increments the label count after the fact.
		iii. The if statement is then added to the list of functions after ") goto label1;" is appended.
	d. The next statement after the if statement is a goto statement, which consists of "goto label2;" and is added to the function list.
	e. The first label statement, "label1:;", must be pushed next.
	f. The function must check for block statements after the first label statement to make sure block statements are placed correctly.
	g. The second label statement, "label2:;", is the last element added to the list of functions.
